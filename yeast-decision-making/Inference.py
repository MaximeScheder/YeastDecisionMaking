...
def ABC_SMC(epsilons, X, N, prior, Kernel, system, distance, update_kernel = True, maxiter = 1e4):
    """
    ...
    for t, eps in enumerate(epsilons):
        ...
        while i <= N-1:
            ...
            if niter == maxiter:
                print("/!\ Maximum iteration reached : ending process")
                return thetas[:,:,:t], weights[:,:t]
            ...
            if d <= eps:
                ...
                print("\t \t Particle {} accepted with d = {:.2f}".format(i+1, d))
                print("\t Now computing its weight...")
